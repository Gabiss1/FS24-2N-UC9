...
public class ListaPokemonsPanel extends JInternalFrame {
    ...
    private JButton btnAtualizar, btnRemover, btnBuscar, btnEditar, btnBuscarTreinador;
    private JTextField txtBuscaNome, txtBuscarTreinador;
    ...
    public ListaPokemonsPanel(PokemonController controller) {
        ...
        txtBuscarTreinador = new JTextField(20);
        btnBuscar = new JButton("Buscar por Pokemon");
        btnBuscarTreinador = new JButton("Buscar por Treinador");
        ...
        panelAcoes.add(new JLabel("ID Treinador:"));
        panelAcoes.add(txtBuscarTreinador);
        panelAcoes.add(btnBuscar);
        panelAcoes.add(btnBuscarTreinador);
        ...
        btnBuscarTreinador.addActionListener(e -> buscarPorTreinador());
    ...
    private void buscarPorTreinador() {
        int idBusca = Integer.parseInt(txtBuscarTreinador.getText().trim());
        tableModel.setRowCount(0);

        List<Pokemon> pokemons = controller.buscarTreinadorPorNome(idBusca);
        System.out.println(pokemons.getFirst().getNome());

        if (pokemons.isEmpty()) {
            JOptionPane.showMessageDialog(this, "Nenhum Treinador encontrado com o ID: '" + idBusca + "'", "Busca", JOptionPane.INFORMATION_MESSAGE);
        }

        for (Pokemon treinador : pokemons) {
            tableModel.addRow(new Object[]{
                    treinador.getId(),
                    treinador.getNome(),
                    treinador.getTipoPrimario(),
                    treinador.getTipoSecundario() != null ? treinador.getTipoSecundario() : "",
                    treinador.getNivel(),
                    treinador.getHpMaximo(),
                    treinador.getFK_Treinador()
            });
        }
    }
